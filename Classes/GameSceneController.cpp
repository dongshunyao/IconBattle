...
void GameSceneController::startLevelGame(const bool isClassical, const int level) const
{
	if (isClassical)
		Director::getInstance()->replaceScene(
			TransitionSlideInR::create(1.0f, GameScene::createScene(classicalLevel[level].second,
			                                                      classicalLevel[level].first, isClassical, LEVEL_MODE,
			                                                      level)));
	else
		Director::getInstance()->replaceScene(
			TransitionSlideInR::create(1.0f, GameScene::createScene(plusLevel[level].second, plusLevel[level].first,
...
void GameSceneController::startChallengeGame(const bool isClassical) const
{
	if (isClassical)
		Director::getInstance()->replaceScene(
			TransitionSlideInR::create(1.0f, GameScene::createScene(classicalChallenge.second, classicalChallenge.first,
